...
interface PresentationState {
  ...
  addSlide: () => void;
  updateSlide: (index: number, elements: any[]) => void;
...
export const useStore = create<PresentationState>((set) => ({
  ...
  updateSlide: (index, elements) => set((state) => {
  ...
  deleteSlide: (index) => set((state) => {
    ...
    return {
      ...
      currentSlideIndex: Math.max(0, state.currentSlideIndex - (index <= state.currentSlideIndex ? 1 : 0))
    };
  }),
  reorderSlides: (fromIndex, toIndex) => set((state) => {
    ...
    return {
      slides: newSlides,
      currentSlideIndex: state.currentSlideIndex === fromIndex ? toIndex : state.currentSlideIndex
    };
  }),
